...
class Face_emotion_detect:
    ...
    def getElderEmotion(self, emo_list, face_list):
            ...
            if str(tuple1[1]) == "elder":
                for tuple2 in emo_list:
                    ...
                    if difference < min_difference:
                        ...
                        # tuple2[1]是当前老人的emo
                        if tuple2[1] != self.last_frame_emo:  # 表情发生切换
                            # 一个字典，存储的是每一个老人自己的名字对应的same emo的count计数
                            elder_same_emo_count[tuple1[2]] = 0
                        else:  # 表情未切换
                            if tuple1[2] not in elder_same_emo_count:  # 初始化
                                elder_same_emo_count[tuple1[2]] = 0
                            elder_same_emo_count[tuple1[2]] += 1
                        if elder_same_emo_count[tuple1[2]] >= smile_limit and tuple2[
                            1] != "Neutral":  # 大于一个阈值的话,并且为一个特殊表情
                            current_time = datetime.now()
                            res.append((tuple1[1], tuple1[2], tuple2[1], elder_same_emo_count[tuple1[2]], current_time))
                            elder_same_emo_count[tuple1[2]] = 0
                        last_frame = tuple2[1]  # 最后更新上一帧的emo
        return res
    ...
    def process(self):
        ...
        if self.get_face_database():
            ...
            while stream.isOpened() and self.open:
                self.frame_cnt += 1
                ...
                if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                    ...
                    res = elderSmileEvent(emo_list, face_list, img_rd)
                    if res:
                        print("检测到的事件是", res)
                        append_data_to_csv("app01/elderSmileEvent.csv", res)
        ...
        if self.get_face_database():
            ...
            if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                ...
                for i in range(self.current_frame_face_cnt):
                    ...
                    frame = cv2.putText(frame,
                                        ...
                                        cv2.LINE_AA)
                    print("人脸的text", text)
                res = elderSmileEvent(emo_list, face_list, frame)
            ...
            # 6.2 如果当前帧和上一帧人脸数发生变化 / If cnt of faces changes, 0->1 or 1->0 or ...
                ...
                # 6.2.2 人脸数增加 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                    ...
                    # 7. 生成的窗口添加说明文字 / Add note on cv2 window
                    self.draw_note(frame)
            event = elderSmileEvent(emo_list, face_list, frame)
            self.update_fps()
...
# 老人的微笑检测
def elderSmileEvent(emo_list, face_list, frame):
    min_difference = float('inf')  # 初始化最小差值为正无穷大
    res = []
    for tuple1 in face_list:
        if str(tuple1[1]) == "elder":
            ...
            for tuple2 in emo_list:
                ...
                if difference < min_difference:
                    ...
                    if elder_smile_count[tuple1[2]] >= smile_limit:
                        current_time = datetime.now()
                        current = str(current_time).replace(" ", "_").replace(":", "_").replace("-", "_").replace(".",
                                                                                                                  "_")
                        filePath = "app01/smileEventImages/" + current + ".jpg"
                        cv2.imwrite(filePath, frame)
                        res.append((1, current_time, filePath, tuple2[1], tuple1[2]))
                        elder_smile_count[tuple1[2]] = 0
