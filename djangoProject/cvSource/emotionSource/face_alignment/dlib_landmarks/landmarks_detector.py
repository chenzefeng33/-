...
class dlibLandmarks(LandmarksDetectorIface):
    ...
    def __init__(self, root='cvSource/emotionSource/face_alignment/dlib_landmarks'):
        self.path = "shape_predictor_5_face_landmarks.dat"
        self.path = os.path.join(root, self.path).replace("\\", "/")
    ...
    def detect_landmarks(self, frame, rect):
        ...
        if type(rect) != dlib.rectangle:
            (x,y,w,h) = rect
            rect = dlib.rectangle(left=x, top=y, right=x, bottom=y)
