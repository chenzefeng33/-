...
class DnnDetector(FaceDetectorIface):
    ...
    def __init__(self, root=None):
        ...
        if root:
            self.prototxt = os.path.join(root, self.prototxt).replace("\\", "/")
            self.model_weights = os.path.join(root, self.model_weights).replace("\\", "/")
        print("当前的path：")
        self.detector = cv2.dnn.readNetFromCaffe(self.prototxt, self.model_weights)
