...
detector = cv2.dnn.readNetFromCaffe("cvSource/emotionSource/data/dnn_detect/deploy.prototxt.txt",
                                    "cvSource/emotionSource/data/dnn_detect/res10_300x300_ssd_iter_140000.caffemodel")


class Face_Register:
    def __init__(self, people_type, name):
        self.init = False
        self.path_photos_from_camera = "cvSource/emotionSource/data/data_faces_from_camera"
    ...
    def pre_work_del_old_face_folders(self):
        ...
        if os.path.isfile("cvSource/emotionSource/data/features_all.csv"):
            os.remove("cvSource/emotionSource/data/features_all.csv")

    # 如果有之前录入的人脸, 在之前 person_x 的序号按照 person_x+1 开始录入 (v1)
    # 根据传入的人员类型和 id 录入 (v2)
    def check_existing_faces_cnt(self):
        if os.listdir("cvSource/emotionSource/data/data_faces_from_camera/"):
            # 获取已录入的最后一个人脸序号
            person_list = os.listdir("cvSource/emotionSource/data/data_faces_from_camera/")
            person_num_list = []
    ...
    def process(self, stream):
        ...
        while stream.isOpened():
            ...
            if kk == ord('n'):
                ...
                current_face_dir = self.path_photos_from_camera + '/' + self.people_type + '/' + self.name
...
# def main():
#     Face_Register_con = Face_Register(2, "yaren")
